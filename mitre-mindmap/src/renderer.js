 ...
 *
 * Dependencies: d3 v7, data.js (DATA global or import)
 *
 * Claude Code tasks:
 ...
 *   - Export renderToSVG(container) to embed in other apps
 */
...
const root_d={id:"root",name:"ATT·CK",type:"root",children:DATA.map(t=>({
  ...
  children:t.techs.map(te=>({
    ...
    tactic:t.name, tacticId:t.id, desc:te.desc, cmds:te.cmds||[], mits:te.mits||[],
...
function showTT(data){
  ...
  titleEl.textContent=data.name; titleEl.style.color=data.color||'#00c8ff';
  let html='';
...
function zoomTo(s){svg.transition().duration(600).call(zoom.transform,d3.zoomIdentity.scale(s));}
...
DATA.forEach(t=>{
  ...
  el.onclick=()=>{const d=hier.descendants().find(n=>n.data.id===t.id);if(d)showTT(d.data);};
  leg.appendChild(el);
});
